    ...
    def _load_model_providers(self):
        ...
        for provider_name, provider_config in providers_config.items():
            if provider_name in MODEL_PROVIDERS:
                ...
                # Register with fusion and voting systems
                self.context_fusion.register_provider(provider_name, provider)
                self.model_voting.register_provider(provider_name, provider)
...
@click.pass_context
def run(ctx, repl, version):
    """Run the Cynetics CLI."""
    if version:
        click.echo("Cynetics CLI v0.1.0")
        return
...
def personality(ctx, list_modes, mode):
    ...
    personality_system = AdaptivePersonality()
    
    if list_modes:
        modes = personality_system.list_modes()
        click.echo("Available personality modes:")
        for mode_name, description in modes.items():
            current_marker = " (current)" if mode_name == personality_system.get_mode().value else ""
            click.echo(f"  {mode_name} - {description}{current_marker}")
    elif mode:
        try:
            agent_mode = AgentMode(mode.lower())
            personality_system.set_mode(agent_mode)
            click.echo(f"Personality mode set to: {mode}")
        except ValueError:
            click.echo(f"Error: Unknown mode '{mode}'. Use --list-modes to see available modes.")
    else:
        current_mode = personality_system.get_mode()
        click.echo(f"Current personality mode: {current_mode.value}")
        click.echo(f"Description: {personality_system.get_mode_description()}")
